...
@pytest.mark.asyncio
async def test_singleton_if_using_type(container: ContainerInterface):
    container.singleton_if(FooInterface, FooImplementation)
    assert container.is_bound(FooInterface)

    container.singleton_if(FooInterface, FooSecondaryImplementation)
...
@pytest.mark.asyncio
async def test_singleton_if_using_callable(container: ContainerInterface):
    container.singleton_if(FooInterface, lambda app, request: FooImplementation())  # type: ignore
    container.singleton_if(
        FooInterface,
        lambda app, request: FooSecondaryImplementation(),  # type: ignore
    )
...
@pytest.mark.asyncio
async def test_singleton_if_resolves_same_instance(container: ContainerInterface):
    container.singleton_if(FooInterface, FooImplementation)
