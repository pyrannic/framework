...
from pyrannic.contracts.orm.repository import RepositoryInterface
...
class Repository(QueryBuilder[T], RepositoryInterface[T]):
    ...
    def destroy(self, model: T | None = None) -> None:
        ...
        try:
            ...
            self.session.rollback()
            self._logger.exception("Error destroying model. Rolling Back...")
            raise
        finally:
            self._reset_query()
    ...
    def count(self) -> int:
        self._prepare_query()
        self._before_query()
        return self._count(reset_query=True)
